...
- When an issue builds on other issues, list those issues' positions in its `builds_on` field (1-based positions in your returned array) — and order the array so they come first: `builds_on` may only point to earlier issues. Never write sequence references like "(1/3)" into titles or bodies yourself — the system renders the final numbering (the `(n/total)` titles and a "Builds on (n/total)." body line) from your order and `builds_on`, and hand-written numbers go stale.
...
- `type` — the work-item code, exactly one of: `BUG` (defect fix), `FEAT` (new functionality), `CR` (change request to existing behaviour), `CONF` (configuration/setup), `DEV` (internal development/refactoring), `MIG` (migration), `SUP` (support task), `DOC` (documentation). **Classify the ticket once, by its overall nature, and set that type on every issue.** A ticket delivering a new capability is `FEAT` for all of its issues — even when individual steps modify existing screens, views, or reports on the way; `CR` only when the ticket itself asks to change behaviour that already exists and was agreed (behaviour that contradicts what was agreed is `BUG`). Per-issue divergence is allowed only where the ticket itself marks parts differently (e.g. classified use-case sections). When the request specifies a fixed type, set that type on every issue.
...
- `anchor_ref` — the id of the anchor story this estimate is based on, copied from the calibration block without the backticks that surround it there, or null when none is comparable.
