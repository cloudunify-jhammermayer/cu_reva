...
# --- test-dir exclusion (full/deep only) --------------------------------------

_DIFF_WITH_TESTS = (
    "diff --git a/custom_addons/mod/models/foo.py b/custom_addons/mod/models/foo.py\n"
    "+++ b/custom_addons/mod/models/foo.py\n"
    "+ real change\n"
    "diff --git a/custom_addons/mod/tests/test_foo.py b/custom_addons/mod/tests/test_foo.py\n"
    "+++ b/custom_addons/mod/tests/test_foo.py\n"
    "+ test change\n"
)
_FILES_WITH_TESTS = [
    {"filename": "custom_addons/mod/models/foo.py"},
    {"filename": "custom_addons/mod/tests/test_foo.py"},
]


@pytest.mark.parametrize("mode", ["full", "deep"])
def test_full_deep_review_excludes_tests_dir(mode):
    github = FakeGitHub(diff=_DIFF_WITH_TESTS, files=list(_FILES_WITH_TESTS))
    runner = FakeRunner(response=_claude_response_with_findings([]))
    reviewer, *_ = _make_reviewer(github=github, runner=runner)
    result = reviewer.execute(_params(review_mode=mode))
    assert result.status == "completed"
    reviewed = runner.last_params["diff"]
    assert "custom_addons/mod/models/foo.py" in reviewed
    assert "tests/test_foo.py" not in reviewed
    assert "tests/test_foo.py" not in runner.last_params["changed_files"]


def test_full_review_declines_when_only_tests_changed():
    diff = (
        "diff --git a/custom_addons/mod/tests/test_foo.py b/custom_addons/mod/tests/test_foo.py\n"
        "+++ b/custom_addons/mod/tests/test_foo.py\n"
        "+ test change\n"
    )
    github = FakeGitHub(diff=diff, files=[{"filename": "custom_addons/mod/tests/test_foo.py"}])
    reviewer, *_ = _make_reviewer(github=github)
    result = reviewer.execute(_params(review_mode="full"))
    assert result.status == "declined"


def test_diff_review_keeps_tests_dir():
    # diff/diff-all are NOT affected — test changes are still reviewed there.
    github = FakeGitHub(diff=_DIFF_WITH_TESTS, files=list(_FILES_WITH_TESTS))
    runner = FakeRunner(response=_claude_response_with_findings([]))
    reviewer, *_ = _make_reviewer(github=github, runner=runner)
    reviewer.execute(_params(review_mode="diff"))
    assert "tests/test_foo.py" in runner.last_params["diff"]
