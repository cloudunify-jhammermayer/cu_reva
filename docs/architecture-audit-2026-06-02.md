...
d
**Progress (2026-06-02):** ✅ **Gate 0 complete** — SECU-1, DEPE-1, SECU-2 (+CORR-9), CORR-1, INFR-1, DOCS-1. **Gate 1 in progress** — ✅ CORR-2, CORR-4, SECU-7, INFR-2, SECU-5, SECU-6, SECU-3, SECU-4 (+CORR-11) done — incl. a unified spend ledger (theme #3) so the daily cap is a pre-flight gate across reviews/audits/replies. Remaining: R10 (backup/restore — ops task, needs a live DB), INFR-3 (graduate CI scanners). CONC-7 (in-flight reservation) intentionally deferred — the cap is a pre-flight gate by design decision, not a reservation.
