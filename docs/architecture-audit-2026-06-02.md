...
- **SECU-8** — **No data-retention/purge** for stored customer content (raw ticket text, full webhook payloads, review output accumulate forever) → GDPR/data-minimisation gap, can't honour erasure, larger breach blast-radius. *(Unverified — critic finding; plausible, line numbers not independently checked.)* *Fix:* periodic purge + retention window; stop storing full payloads/raw text longer than needed.
...
|---|---|---|---|
| R10-restore | **DB backup + restore never verified on host** — `backup.sh` exists, syntax-checked, but no real backup→restore run; no proven recovery path | needs-validation | prod-readiness-plan R10 |
...
- **CONC-8** — Stale-running reaper uses a plain SELECT (no SKIP LOCKED) and wall-clock-only liveness, so concurrent schedulers double-reap and a reaped row can be silently re-completed
...
- **CONC-10** — Monitor alert-firing state is per-process in memory, causing N-times alert/recovery spam across scheduler replicas
...
- **CORR-11** — AuditResult carries no token/cost fields, so audit spend can never be recorded
...
- **DEPE-3** — uv.lock is committed but incomplete and never consumed by any build or CI step
...
- **TEST-7** — scheduler WeeklyReporter and worker audit_tasks.run_audit have no tests despite injectable, deterministic logic
...
- **CODE-7** — Six TUI source files are not gofmt-clean; formatting not enforced in CI  (`tui/internal/api/mock.go, internal/api/types.go, internal/ui/findings.go, internal/ui/messages.go, internal/ui/repos.go, internal/ui/styles.go`)
