...
- **Scope filtering.** Only `custom_addons/` / `custom-addons/` paths reviewed by default (`reva/diff_utils.py`); `.po`/`.pot`/`.md`/`.rst` and `odoo/`/`enterprise/` always stripped — but `custom_addons/**/*.xml` **is** reviewed (Odoo views, via `reva-xml-review`). Per-repo overrides in `.claude-review.yml` (`max_diff_lines`, `max_diff_tokens`, `max_xml_diff_lines`, `max_xml_diff_tokens`, `skip_paths`, `review_all_paths`, `block_on_severity`, `verify_findings`, `cross_branch_reuse`, `code_grounding`, `odoo`, `custom_instructions`).
