...
- **Messages API** (`reva/claude_client.py`) — structured/fast paths: Odoo ticket analysis, Odoo timesheet wording review, support-answer drafts, and inline-comment reply answers. Ticket analysis and support answers can ESCALATE to the CLI path above when the planner flags the question as needing project code (see the code-grounding invariant). Prompts assembled by task-specific analyzers from `prompts/*.md` with prompt-cache–controlled blocks.
...
- `prompts/skills/` — the headless-CLI skills. Selection is centralized in `Reviewer._select_skill` on the final (post-filter) diff, precedence **migration > delta > xml-only > diff/full**: `reva-diff-review` (diff/diff-all), `reva-full-review` (full/deep), `reva-delta-review` (incremental), `reva-migration-review` (Odoo upgrade scripts — overrides mode/delta, keeps `delta_base_sha`), `reva-xml-review` (XML-only diff), `reva-repo-audit` (audits). Two skills sit OUTSIDE that selection because they answer a question rather than review a diff, and are named directly by their runners: `reva-support-answer` and `reva-ticket-analysis` (planner-gated code grounding, see below). CodeGraph MCP (`REVA_CODEGRAPH_ENABLED`, fail-silent) is wired into the repo-aware skills only (full/deep/audit + those two), never the diff-depth paths. `_NO_REVIEW_GUIDANCE_SKILLS` keeps `review_guidance.md` — severity/category/confidence/summary governance for *findings* — off the two non-findings skills, where it would be ~3k wasted tokens and contradictory instructions.
...
- **Delta scope & cross-branch reuse** (spec `docs/superpowers/specs/archive/2026-07-24-force-push-delta-and-cross-branch-reuse-design.md`). A re-push that *diverged* from the last-reviewed SHA (force-push/amend) still gets a delta: `Reviewer` uses `ClaudeCodeRunner.two_tree_diff` (a **lock-free, read-only** local `git diff <prior> <new>`, degrade-in-place) gated on the merge-base with the target being unchanged; base moved / objects gone → full review. `review_runs.diff_hash` (index-line-stripped hash of the filtered diff; NULL on delta runs, tracked via `delta_base_sha`) fingerprints full-scope reviews; on a PR's **first** review a same-repo `diff_hash` match is **carried forward** (prior PR's open findings, current mutes applied, `carried_from_run_id` set) with no Claude call. Global kill switch `REVA_CROSS_BRANCH_REUSE`, per-repo `cross_branch_reuse`; explicit comment/requeue triggers never carry forward. Deferred: `content_hash` matching, cross-branch *partial* delta, core-version gating.
- **Planner-gated code grounding** (spec `docs/superpowers/specs/archive/2026-07-25-support-answers-design.md`). `plan_core_queries` emits `needs_repo_code`, carried on `TicketKnowledge`; it is independent of `worth_checking` (a question can need the project's code while the official docs are irrelevant). True → one headless-CLI run under `repo_lock` against the worker clone (`reva-ticket-analysis` / `reva-support-answer`); false → the Messages API path. Escalation costs roughly 10-30x and takes the per-repo lock, so the gate is **shut on any doubt** — planner failure, missing key, no `github_url`, App not installed, kill switch — and every shut reason is an ops event. Brakes: global `REVA_TICKET_CODE_GROUNDING`, per-repo `code_grounding`.
...
- **Scope filtering.** Only `custom_addons/` / `custom-addons/` paths reviewed by default (`reva/diff_utils.py`); `.po`/`.pot`/`.md`/`.rst` and `odoo/`/`enterprise/` always stripped — but `custom_addons/**/*.xml` **is** reviewed (Odoo views, via `reva-xml-review`). Per-repo overrides in `.claude-review.yml` (`max_diff_lines`, `max_diff_tokens`, `max_xml_diff_lines`, `max_xml_diff_tokens`, `skip_paths`, `review_all_paths`, `block_on_severity`, `verify_findings`, `cross_branch_reuse`, `code_grounding`, `odoo`, `custom_instructions`, `odoo_instance`).
