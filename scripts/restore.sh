...
echo "==> About to restore ${file} into the 'reviews' database."
echo "    THIS OVERWRITES the current database (objects are dropped + recreated)."
...
gunzip -c "$file" \
  | $COMPOSE exec -T postgres psql -v ON_ERROR_STOP=1 --single-transaction -U review -d reviews

echo "==> Restore complete. Sanity check:"
$COMPOSE exec -T postgres psql -U review -d reviews -tAc \
